...
app.post("/delete", async (req, res) => {
  ...
  try {
      await db.collection("items").deleteMany({ name, description }); // Use variables name and description
      res.redirect("/");
  } catch (err) {
...
app.get("/report", async (req, res) => {
    try {
      ...
      let tableContent = "<h1>Report</h1><table style='coloer:red' border='1'><tr><th>Name</th><th>Description</th></tr>";
